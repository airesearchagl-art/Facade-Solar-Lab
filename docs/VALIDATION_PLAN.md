...
- M4 / M4.5 / M6 / M7: COMPLETE。M5: local検証完了 / external reference待ち。M7はIndependent FULL Re-Review A. PASS / Required Fix CLOSED、exact product headのHuman HTTP・Single/Multi CSV/PDF PASSに加え、PR #12 squash mergeとmain `642136058d538e89d29971b0a586f86ea3aaa926`のGit Production provenance・HTTP/browser post-merge gate PASS。詳細は[受入証拠](ADVANCED_FACADE_SHADING.md#human-acceptance--independent-re-review)と[Production closeout](ADVANCED_FACADE_SHADING.md#post-merge-production-closeout)。M8はGuide implementation complete、Independent FULL Review A. PASS / Required Fix NONE、Human UX Review（manual print / PDF visual acceptanceを含む）PASS、PRE-MERGE GATE PASS。PR #13はOPEN / Draftで、Ready / merge / post-merge Production confirmationはpending。§10のPR/Ready/M6状態は当時の履歴として保持し、最新運用状態はROADMAPに従います。
