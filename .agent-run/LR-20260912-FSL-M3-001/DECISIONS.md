...
| M3-D-009 | Exclude only the immutable M3 Task Packet snapshot from whitespace findings via path-specific `.gitattributes` | Its normalized final LF is part of the bound bytes and cannot be edited after checkpoint | Required range diff check remains effective for every other changed file while snapshot digest stays exact |
