...
export function MultiFloorWorkspace() {
  ...
  return (
    <main className="app-shell multifloor-shell">
      <header className="app-header"><div><p className="eyebrow">M4.5 · 複数階モード</p><h1>Facade Solar Lab</h1><p className="lede">既存の単一階計算を各Floorへ再利用し、建物全体と階別の日射熱取得を比較します。</p></div><div className="model-chip"><span aria-hidden="true" />各階で facade-v1-weather</div></header>
      ...
      <section className="panel multifloor-case-panel" aria-labelledby="multifloor-cases-title">
        ...
        <div className="multifloor-workspace-grid">
          ...
          <section className="preview-workspace" aria-labelledby="multifloor-geometry-title"><div className="subsection-heading"><div><p className="section-kicker">03 · 積層形状</p><h2 id="multifloor-geometry-title">{selectedCase.name}</h2></div><span>{selectedCase.floors.length} Floors</span></div><MultiFloorGeometryPreview buildingCase={selectedCase} selectedFloorId={selectedFloor.id} dataset={dataset} /></section>
        </div>
      </section>
