    ...
    <section className="panel multifloor-results" aria-labelledby="multifloor-results-title">
      ...
      <section className="print-only multifloor-all-floor-report" aria-label="全建物案の階別結果">
        {result.cases.map((item, index) => <article key={item.caseId} className="multifloor-case-report">
          ...
          <h3>階別の遮蔽入力</h3>
          <table className="data-table floor-shading-inputs"><thead><tr><th>階</th><th>水平庇 [m]</th><th>端部・中間フィン（階床基準）</th></tr></thead><tbody>{item.floors.map((floor) => <tr key={floor.floorId}><th>{floor.name}</th><td>{floor.definition.overhang === undefined ? "なし" : `出 ${floor.definition.overhang.depthM} / 高さ ${floor.definition.overhang.elevationM} / 左右延長 ${floor.definition.overhang.leftExtensionM}, ${floor.definition.overhang.rightExtensionM}`}</td><td><FinSummary fins={floor.definition} widthM={floor.definition.opening.widthM} /></td></tr>)}</tbody></table>
